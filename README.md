...
2. [Overview](#overview)
   ...
   - [RealtimeClient](#realtimeclient-interface)
...
```ts
export default new IntegerUtils();
```
...
```ts
import StringUtils from "@commonmodule/ts/utils/StringUtils.js";
...
```ts
import ObjectUtils from "@commonmodule/ts/utils/ObjectUtils.js";
...
```ts
import Debouncer from "@commonmodule/ts/utils/Debouncer.js";
...
```ts
import EventContainer from "@commonmodule/ts/event/EventContainer.js";

interface MyEvents {
  dataReceived: (data: any) => void;
}

const eventContainer = new EventContainer<MyEvents>();

eventContainer.on("dataReceived", (data) => {
  console.log("Data received:", data);
});

// Emit an event
eventContainer.emit("dataReceived", { message: "Hello" });
...
```ts
import MessageChannelManager from "@commonmodule/ts/communication/MessageChannelManager.js";
import RealtimeClient from "@commonmodule/ts/communication/RealtimeClient.js";
